    ...
    def DateUpdate(self, tag_day:str=None, org_id:int=None, db_id:int=None,file_dir:str=None, programe="shell",from_db_type=None,to_db_type=None):
        ...
        if programe=="shell":
            if self.product=='nix':            
                ...
                file_dir = self.dir_file_folder + "/nix_self_p1.sh" if file_dir==None else file_dir
                ...
                file_dir = self.dir_file_folder + "/cdp_self_p1.sh" if file_dir==None else file_dir
...
class TaggablesMethod:
    def __init__(self, product=None, org_id=None, db_id=None,db_type=None):
    ...
    def TriggerTaggables(self, dir_file,tag_day=None):
        ...
        if self.product=='nix':
            ...
            ### 格式： {sh檔路徑}{參數1:從何處輸入並輸出資料庫}{參數2:統計日}{參數3:廠商db}{參數4:error log 輸出路徑}
            os.system('sh ' f'{dir_file} {self.db_type} {tag_day} {self.org_id} {self.error_log_cdp}')  
...
if __name__ == '__main__':
    ...
    sql="select id from accunix_v2.linebots"
    df_dbID = DbConnRead_df(sql=sql, db_name='nix')

    for db_id in [19,62,100]:#df_dbID.id:
        # db_id =66
        #這邊要注意 tag_day 是也有被收錄的，所以迴圈這邊當下的時間，最後送進去的是昨天
        """先對天更新"""
        for dd in day_range:
            try:            
                clss_TriggerEtl = TriggerETL(product='nix',from_db_type='nix',to_db_type='datapool') #輸出到datapool
                clss_TriggerEtl.MapPath(map_dict=dict_Path) #建立路徑
                clss_TriggerEtl.DateUpdate(tag_day=dd, db_id=db_id)
                print(dd)
            except Exception as ex:
                #TODO:log檔案紀錄還未設定
                print(ex)
        ...
        """製作90天內的 taggables"""
        class_TriggerTaggables= TaggablesMethod(product='nix', db_id=db_id, db_type='datapool')
        dir_file = dict_Path['dir_file_folder'] + "/nix_self_p2.sh"
        class_TriggerTaggables.MapPath(map_dict=dict_Path)
        class_TriggerTaggables.TriggerTaggables(dir_file=dir_file, tag_day=dd)
        
        """製作儀表板"""
        dir_file = dict_Path['dir_file_folder'] + "/nix_self_p3.sh"
        db_type='datapool'
        error_log = dict_Path['error_log_nix']
        os.system('sh ' f'{dir_file} {db_type} {dd} {db_id} {error_log}')  
        # breakpoint()
