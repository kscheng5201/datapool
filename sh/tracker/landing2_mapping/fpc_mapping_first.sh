...
echo 'start: ' `date` 
echo 'export data for src_fpc_mapping_first_'${i}
mysql --login-path=$dest_login_path -e "LOAD DATA LOCAL INFILE '$export_dir/$src_login_path_2/$project_name/$project_name.${table_name}.txt' IGNORE INTO TABLE ${src_login_path}.cdp_fpc_mapping IGNORE 1 LINES;" 2>>$error_dir/$project_name.src_fpc_mapping_first_${stakeholder}.error 
