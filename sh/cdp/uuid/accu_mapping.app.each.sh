...
do 
    echo ''
    echo [beforehand: DROP TABLE ${project_name}.${table_name}_app_${org_id}_src]
    mysql --login-path=$dest_login_path -e "DROP TABLE ${project_name}.${table_name}_app_${org_id}_src;"
    echo ''
    echo [beforehand: DROP TABLE ${project_name}.${table_name}_app_${org_id}_mid]
    mysql --login-path=$dest_login_path -e "DROP TABLE ${project_name}.${table_name}_app_${org_id}_mid;"
    echo ''
    echo [beforehand: DROP TABLE IF EXISTS ${project_name}.${table_name}_${org_id}_mid_bfpc_f]
    mysql --login-path=$dest_login_path -e "DROP TABLE IF EXISTS ${project_name}.${table_name}_app_${org_id}_mid_bfpc_f;"
    echo ''
    ...
    export sql_2="
        ...
        ;
        CREATE TABLE IF NOT EXISTS ${project_name}.${table_name}_app_${org_id}_mid_bfpc_f (
            accu_id varchar(36) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '由 ##mysql uuid() 機制所產生的 id',
            browser_fpc varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '透過 tracker 比對後的指紋碼 fingerprint code',            
	...
	;"
    echo ''
    echo [整理 ${project_name}.${table_name}_app_${org_id}_mid：JOIN 同個 browser_fpc，建立寫入 ${project_name}.${table_name}_app_${org_id} 的預備表]
    echo [INSERT INTO ${project_name}.${table_name}_app_${org_id}_mid]
    echo $sql_4
    mysql --login-path=$dest_login_path -e "$sql_4" 2>>$error_dir/$src_login_path/$project_name/$project_name.${src_login_path}_${table_name}_app_${org_id}_mid.error        


    export sql_5b="     
        INSERT INTO ${project_name}.${table_name}_app_${org_id}_mid_member_f
    ...
    echo $sql_5b
    mysql --login-path=$dest_login_path -e "$sql_5b" 2>>$error_dir/$src_login_path/$project_name/$project_name.${src_login_path}_${table_name}_app_${org_id}_mid.error        
