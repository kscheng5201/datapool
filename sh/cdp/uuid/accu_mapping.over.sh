...
echo ''
echo [beforehand: DROP TABLE ${project_name}.${table_name}_mid]
...
echo $sql_2
mysql --login-path=$dest_login_path -e "$sql_2" 2>>$error_dir/$src_login_path/$project_name/$project_name.${src_login_path}_${table_name}_${org_id}_src.error
...
export sql_4="            
    UPDATE ${project_name}.${table_name}_mid a
        INNER JOIN
        (
        select accu_id, browser_fpc, rid
        from (
