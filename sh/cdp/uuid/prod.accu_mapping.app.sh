...
do 
    echo ''
    echo [beforehand: DROP TABLE IF EXISTS ${project_name}.${table_name}_app_${org_id}_src]
    mysql --login-path=$dest_login_path -e "DROP TABLE IF EXISTS ${project_name}.${table_name}_app_${org_id}_src;"
    echo ''
    echo [beforehand: DROP TABLE IF EXISTS ${project_name}.${table_name}_app_${org_id}_mid]
    mysql --login-path=$dest_login_path -e "DROP TABLE IF EXISTS ${project_name}.${table_name}_app_${org_id}_mid;"
    echo ''
    echo [beforehand: DROP TABLE IF EXISTS ${project_name}.${table_name}_${org_id}_mid_member_f]
    mysql --login-path=$dest_login_path -e "DROP TABLE IF EXISTS ${project_name}.${table_name}_app_${org_id}_mid_member_f;"
    ...
    echo $sql_1
    mysql --login-path=${src_login_path}_master -e "$sql_1" > $export_dir/$src_login_path/$project_name/$project_name.${src_login_path}_${table_name}_app_${org_id}_db_id.txt 2>> $error_dir/$src_login_path/$project_name/$project_name.${src_login_path}_${table_name}_app_${org_id}_db_id.error
    sed -i '1d' $export_dir/$src_login_path/$project_name/$project_name.${src_login_path}_${table_name}_app_${org_id}_db_id.txt


    export sql_2="
        CREATE TABLE IF NOT EXISTS ${project_name}.${table_name}_app_${org_id}_src (
            ...
            channel varchar(16) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'web(1),line(2),messenger(3),app(4),app(5)',
        ...
        ;
        CREATE TABLE IF NOT EXISTS ${project_name}.${table_name}_app_${org_id}_mid (
            serial int NOT NULL AUTO_INCREMENT COMMENT 'auto_increment Serial Number',
            accu_id varchar(36) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '由 ##mysql uuid() 機制所產生的 id',
            channel varchar(16) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'web(1),line(2),messenger(3),app(4),app(5)',
            org_id int NOT NULL DEFAULT '0' COMMENT '組織/客戶 id',
        ...
        ;
        CREATE TABLE IF NOT EXISTS ${project_name}.${table_name}_app_${org_id}_mid_member_f (
        ...
        ;"        
    echo ''
    echo [CREATE TABLE IF NOT EXISTS ${project_name}.${table_name}_app_${org_id}_src]
    echo $sql_2
    mysql --login-path=$dest_login_path -e "$sql_2" 2>>$error_dir/$src_login_path/$project_name/$project_name.${src_login_path}_${table_name}_app_${org_id}_src.error
    ...
    do 
        export sql_3="
            select 
                ...
                'app' id_type,
                a.member_id, 
                from_unixtime(a.app_unique_created_at) first_at,
                from_unixtime(if(c.registered_at >= 1 and c.registered_at < c.created_at, c.registered_at, c.created_at)) registered_at,
                from_unixtime(a.app_unique_created_at) created_at, 
                from_unixtime(a.updated_at) updated_at
            from cdp_app_${db_id}.app_unique_data a
            
                inner join cdp_app_${db_id}.app_unique b
                on a.app_unique_id = b.id
            ...
            where (a.app_unique_created_at >= unix_timestamp('${vDate}')
                and a.app_unique_created_at < unix_timestamp('${vDate}' + interval 1 day))
        ...
        #### Export Data ####
        echo ''
        echo [Export ${vDate} Data FROM cdp_app_${db_id}.fpc_unique_data and cdp_app_${db_id}.app_unique]
        echo $sql_3
        mysql --login-path=$src_login_path -e "$sql_3" > $export_dir/$src_login_path/$project_name/${project_name}.${src_login_path}_${table_name}_app_${org_id}_${db_id}_src.txt 2>>$error_dir/$src_login_path/$project_name/${project_name}.${src_login_path}_${table_name}_app_${org_id}_${db_id}_src.error
   
        #### Import Data ####
        echo ''
        echo [INSERT ${vDate} data INTO ${project_name}.${table_name}_app_${org_id}_src]
        mysql --login-path=$dest_login_path -e "LOAD DATA LOCAL INFILE '$export_dir/$src_login_path/$project_name/${project_name}.${src_login_path}_${table_name}_app_${org_id}_${db_id}_src.txt' INTO TABLE ${project_name}.${table_name}_app_${org_id}_src IGNORE 1 LINES;" 2>>$error_dir/$src_login_path/$project_name${project_name}.${table_name}_app_${org_id}_src.error 

    done < $export_dir/$src_login_path/$project_name/$project_name.${src_login_path}_${table_name}_app_${org_id}_db_id.txt


    export sql_4a="            
        INSERT IGNORE INTO ${project_name}.${table_name}_app_${org_id}_mid
            select 
                null, 
                a.accu_id, 
                a.channel, 
                ...
                now() created_at,
                now() updated_at
            from ${project_name}.${table_name}_app_${org_id}_src a
        ...
        ;"
    echo ''
    echo [INSERT IGNORE INTO ${project_name}.${table_name}_app_${org_id}_mid]
    echo $sql_4a
    mysql --login-path=$dest_login_path -e "$sql_4a" 2>>$error_dir/$src_login_path/$project_name/$project_name.${table_name}_app_${org_id}_mid_4a.error        
    ...
    echo [INSERT IGNORE INTO ${project_name}.${table_name}_app_${org_id}_mid]
    echo $sql_4b
    mysql --login-path=$dest_login_path -e "$sql_4b" 2>>$error_dir/$src_login_path/$project_name/$project_name.${table_name}_app_${org_id}_mid_4b.error       
    ...
    echo [INSERT IGNORE INTO ${project_name}.${table_name}_app_${org_id}_mid]
    echo $sql_4c
    mysql --login-path=$dest_login_path -e "$sql_4c" 2>>$error_dir/$src_login_path/$project_name/$project_name.${table_name}_app_${org_id}_mid_4c.error       
    ...
    export sql_5a="            
        INSERT INTO ${project_name}.${table_name}_${org_id}
            select 
                ...
                now() created_at, 
                now() updated_at
            from ${project_name}.${table_name}_app_${org_id}_mid
    ...
    echo [INSERT INTO ${project_name}.${table_name}_${org_id}]
    echo $sql_5b
    mysql --login-path=$dest_login_path -e "$sql_5b" 2>>$error_dir/$src_login_path/$project_name/$project_name.${table_name}_${org_id}_4b.error       


    export sql_6="     
        INSERT IGNORE INTO ${project_name}.${table_name}_app_${org_id}_mid_member_f
            select 
                accu_id, 
                member_id, 
                registered_at, 
        ...
        DELETE
        FROM ${project_name}.${table_name}_app_${org_id}_mid_member_f
        WHERE rid >= 2
        ;

        UPDATE ${project_name}.${table_name}_${org_id} a
            INNER JOIN ${project_name}.${table_name}_app_${org_id}_mid_member_f c
    ...
    echo $sql_6
    mysql --login-path=$dest_login_path -e "$sql_6" 2>>$error_dir/$src_login_path/$project_name/$project_name.${src_login_path}_${table_name}_app_${org_id}_mid_sql_6.error 
