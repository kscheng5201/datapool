...
def connect_to_CDP_mysql_and_return_df(query):
    host = 'product-accucdp-rds-cdp-master-prd-01.cfilkryz7gav.ap-northeast-1.rds.amazonaws.com'
    user = 'etl-reader'
    passwd = '6EsuJizi5u'
...
# 檢查中位數是否正常
    ...
    final_new_ssn.loc[(final_new_ssn.create_interval <=7 ,'Kind_of_person')] = 'NEW'
    total_member = len(final_new_ssn['Kind_of_person'])
...
#             final_new_ssn['Kind_of_person'][i] = "NEW"
    ...
    def find_zero(label):
        try:
            return int(len(final_new_ssn['Kind_of_person'][final_new_ssn['Kind_of_person']==label])/total_member*100)
    ...
    table3 = {
     ...
     "N_sum" : len(final_new_ssn['Kind_of_person'][final_new_ssn['Kind_of_person']=="NEW"]),
     "E0_prop": find_zero("E0"),
     "S1_prop": find_zero("S1"),
     "S2_prop": find_zero("S2"),
     "S3_prop": find_zero("S3"),
     "N_prop" : find_zero("NEW")     
    }    
...
# =============================================================================
def upload_or_take_from_3DM_DB(query , method ,value):
    host = ' accu-data-science-rds-etl-datapool-prd-01.cfilkryz7gav.ap-northeast-1.rds.amazonaws.com'
...
def df_trasform_query_and_upload(df_data,table_name):
    column_name = ''.join(["`"+name+"`," for name in df_data.columns])[0:-1]
    query = "Insert into "+table_name+"("+column_name+") values"+"(" +("%s,"*len(df_data.columns))[0:-1]+")"
    upload_or_take_from_3DM_DB(query , "add_a_lot" ,tranform_df_to_tuple_for_mysql(df_data.fillna(np.nan).replace([np.nan], [None])))
    print(query)

def create_3table_in_nes_mondel(org_id):    
    upload_or_take_from_3DM_DB("create table app.nes_"+str(org_id)+" like app.nes_11","normal","")
    upload_or_take_from_3DM_DB("create table app.nes_"+str(org_id)+"_etl like app.nes_11_etl","normal","")
    upload_or_take_from_3DM_DB("create table app.nes_"+str(org_id)+"_src like app.nes_11_src","normal","")

def drop_3table_in_nes_mondel(org_id):    
    upload_or_take_from_3DM_DB("drop table app.nes_"+str(org_id)+" ","normal","")
    upload_or_take_from_3DM_DB("drop table app.nes_"+str(org_id)+"_etl ","normal","")
    upload_or_take_from_3DM_DB("drop table app.nes_"+str(org_id)+"_src ","normal","")
