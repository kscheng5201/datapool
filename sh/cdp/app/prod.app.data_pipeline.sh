...
do 
    ...
    echo $sql_1
    mysql --login-path=$dest_login_path -e "$sql_1" 2>>$error_dir/$src_login_path/$project_name/$project_name.${type}_${table_name}_${org_id}_etl_sql_1.error
    ...
    export sql_2="
        INSERT INTO ${project_name}.data_pipeline
            select 
                null serial,
                '${vDate}' + interval 1 day tag_date, 
                '${table_name}_raw_data_${org_id}' table_name, 
                '${org_id}' org_id, 
                '${vDate}' data_dt, 
                count(*) entries, 
                max(updated_at) finished_at, 
                now() created_at, 
                now() updated_at
            from ${project_name}.${table_name}_raw_data_${org_id}
    ...
    echo [INSERT INTO ${project_name}.data_pipeline]
    echo $sql_2
    mysql --login-path=$dest_login_path -e "$sql_2" 2>>$error_dir/$src_login_path/$project_name/$project_name.${type}_${table_name}_${org_id}_etl_sql_2.error
