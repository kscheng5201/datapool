        ...
        export sql_2=" 
            INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_src
                select 
                    ...
                    (select id from ${project_name}.funnel where org_id = ${org_id} and campaign_id = $(echo ${campaign_detail} | cut -d _ -f 1) and is_default = 1) funnel_id, 
                    layer_id,
                    count(*) user, 
                    now() created_at, 
                    now() updated_at
                from (
                    ...
                    where a.campaign_id = $(echo ${campaign_detail} | cut -d _ -f 1)
