...
do 
    ...
    export sql_0="
        ...
        ;
        CREATE TABLE IF NOT EXISTS ${project_name}.${type_s}_${table_name}_${org_id}_src_log (
            serial int auto_increment NOT NULL COMMENT 'auto_increment Serial Number' unique, 
            domain varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '來源 domain',
            campaign_id int(11) unsigned NOT NULL DEFAULT '0' COMMENT 'campaign 編號',
            utm_id int(11) NOT NULL DEFAULT '0' COMMENT 'campaign_utm 流水編號',            
	    ...
	    session varchar(10) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'session/工作階段',
            ...
            KEY idx_utm_id (utm_id) USING BTREE, 
            KEY idx_domain (domain) USING BTREE
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT=DYNAMIC COMMENT='依照 utm 取得接觸過 event 的 fpc_raw_data 全表資訊(歷史資料)，fpc 已 mapping' 
        ;"
    echo ''
    ...
    export sql_3="
        INSERT INTO ${project_name}.${type_s}_${table_name}_${org_id}_src_log
            select 
                null serial, 
                a.domain, 
                a.campaign_id, 
                a.utm_id, 
                ifnull(b.fpc, a.fpc) fpc,
                type, 
                event, 
                col1, 
                col2, 
                ...
                a.created_at, 
                now() updated_at
            from ${project_name}.${type_s}_${table_name}_${org_id}_src a
                left join uuid.cdp_fpc_mapping b
                    on a.fpc = b.origin_fpc and b.domain = substring_index(a.domain, '/', 1)
                left join ${project_name}.${type_s}_both_${org_id}_etl c
                    on c.fpc = ifnull(b.fpc, a.fpc) and a.domain = b.domain and a.created_at = c.created_at
            group by 
                a.domain, 
                a.campaign_id, 
                a.utm_id, 
        ...
        ;"                
    echo ''
    echo [INSERT INTO ${project_name}.${type_s}_${table_name}_${org_id}_src_log]
    echo $sql_3
    mysql --login-path=$dest_login_path -e "$sql_3" 2>>$error_dir/$src_login_path/$project_name/${project_name}.${type_s}_${table_name}_${org_id}_src_log.error
