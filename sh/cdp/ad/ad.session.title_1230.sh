...
do 
    ...
    export sql_2a="
        INSERT INTO ${project_name}.${type_p}_${table_name}_${org_id}_mid
            select 
                null serial, 
                '${vDate}' + interval 1 day tag_date, 
                'daily' span, 
            ...
            from (
                ...
                from ${project_name}.${type_p}_both_${org_id}_etl
                where session_type = 'valid'
                group by page_title, campaign_id, utm_id
                ) b
        ...
        ;"                
    echo ''
    echo [INSERT INTO ${project_name}.${type_p}_${table_name}_${org_id} by utm_id]
    echo $sql_2a
    mysql --login-path=$dest_login_path -e "$sql_2a" 2>>$error_dir/$src_login_path/$project_name/${project_name}.${type_p}_${table_name}_${org_id}_sql2a.error 
    ...
    export sql_3a="
        ...
        ;"                
    echo ''
    echo [INSERT INTO ${project_name}.${type_p}_${table_name}_${org_id} by campaign_id]
    echo $sql_3a
    mysql --login-path=$dest_login_path -e "$sql_3a" 2>>$error_dir/$src_login_path/$project_name/${project_name}.${type_p}_${table_name}_${org_id}_sql3a.error 
    ...
    export sql_7="
        INSERT INTO ${project_name}.${type_s}_${table_name}_${org_id}
            select 
                ...
                sum(valid) valid, 
                sum(invalid) invalid, 
                round(100 * sum(valid) / (sum(valid) + abs(sum(invalid))), 2) valid_ratio,
                round(100 * abs(sum(invalid)) / (sum(valid) + abs(sum(invalid))), 2) invalid_ratio,                
                row_number () over (partition by campaign_id, utm_id order by sum(valid) desc) ranking, 
                null time_flag,
                now() created_at, 
                now() updated_at            
            from ${project_name}.${type_s}_${table_name}_${org_id}
        ...
        ;"
    echo ''
    echo [INSERT INTO ${project_name}.${type_s}_${table_name}_${org_id} where span = 'FULL' and utm_id <> 0]
    echo $sql_7
    ...
    export sql_8="
        INSERT INTO ${project_name}.${type_s}_${table_name}_${org_id}
            select 
                ...
                round(100 * sum(valid) / (sum(valid) + abs(sum(invalid))), 2) valid_ratio,
                round(100 * abs(sum(invalid)) / (sum(valid) + abs(sum(invalid))), 2) invalid_ratio,                
                row_number () over (partition by campaign_id order by sum(valid) desc) ranking, 
                null time_flag,
                now() created_at, 
                now() updated_at            
            from ${project_name}.${type_s}_${table_name}_${org_id}
            where span = 'daily'
                and campaign_start >= '${vDate}'
                and campaign_end <= '${vDate}'
                and utm_id = 0
            group by 
                campaign_id, 
    ...
    echo [INSERT INTO ${project_name}.${type_s}_${table_name}_${org_id} where span = 'FULL' and utm_id = 0]
    echo $sql_8
    mysql --login-path=$dest_login_path -e "$sql_8" 2>>$error_dir/$src_login_path/$project_name/${project_name}.${type_s}_${table_name}_${org_id}_sql8.error 
    ...
    echo $sql_9
    mysql --login-path=$dest_login_path -e "$sql_" 2>>$error_dir/$src_login_path/$project_name/${project_name}.${type_s}_${table_name}_${org_id}_sql9.error 
