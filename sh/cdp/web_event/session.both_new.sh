...
do 
    ...
    mysql --login-path=$dest_login_path -e "truncate table ${project_name}.${type}_${table_name}_${org_id}_etl;"
    ...
    do 
        ...
        do 
            export sql_5="
                ...
                select 
                    null serial, 
                    fpc, 
                    from_unixtime(created_at, '%Y-%m-%d %H:%m:%s') + interval 8 hour created_at, 
                    source domain,
                    'page_view' behavior,  
                ...
                mysql --login-path=$src_login_path -e "$sql_5" > $export_dir/$src_login_path/$project_name/${project_name}.${table_name}_${org_id}_${db_id}_${referrer_detail}_src.txt 2>>$error_dir/$src_login_path/$project_name/${project_name}.${table_name}_${org_id}_${db_id}_${referrer_detail}_src.error
        ...
        export sql_9="
            ...
            select 
                null serial, 
                fpc, 
                from_unixtime(a.created_at, '%Y-%m-%d %H:%m:%s') + interval 8 hour created_at, 
                domain,
                'event' behavior,  
                null traffic_type, 
                null referrer, 
    ...
    echo ''
    echo [DROP src table at the end]
