            ...
            INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_title
                ...
                from (
                    ...
                    where start_date >= STR_TO_DATE(CONCAT(yearweek('${vDate}', 7),' Monday'), '%X%V %W')
                        and start_date < '${vDate}' + interval 1 day
                        and session_type = 'p_invalid'
STR_TO_DATE(CONCAT(yearweek('${vDate}', 7),' Monday'), '%X%V %W')                    group by page_title, domain
                    ) b
                    on a.domain = b.domain
            ...
            # add the no data domain on weekly ones
            INSERT IGNORE INTO ${project_name}.${type}_${table_name}_${org_id}_landing 
                (tag_date, span, start_date, end_date, landing)
                
                select '${vDate}' + interval 1 day, 'weekly', STR_TO_DATE(CONCAT(yearweek('${vDate}', 7),' Monday'), '%X%V %W'), '${vDate}', b.domain
                from (
                    select *
                    from ${project_name}.${type}_${table_name}_${org_id}_landing
            ...
            #INSERT IGNORE INTO ${project_name}.${type}_${table_name}_${org_id}_landing 
                #(tag_date, span, start_date, end_date, landing)
            #Values ('${vDate}' + interval 1 day, 'weekly', STR_TO_DATE(CONCAT(yearweek('${vDate}', 7),' Monday'), '%X%V %W'), '${vDate}', '#')
            ;
            ALTER TABLE ${project_name}.${type}_${table_name}_${org_id}_landing AUTO_INCREMENT = 1
            ; 
    ...
    then 
        export sql_4="
            ...
            # add the no data domain on monthly ones
            INSERT IGNORE INTO ${project_name}.${type}_${table_name}_${org_id}_landing 
                (tag_date, span, start_date, end_date, landing)
                
                select '${vDate}' + interval 1 day, 'monthly', '${vMonthFirst}', '${vDate}', b.domain
                from (
                    select *
                    from ${project_name}.${type}_${table_name}_${org_id}_landing
                    where tag_date = '${vDate}' + interval 1 day
            ...
            #INSERT IGNORE INTO ${project_name}.${type}_${table_name}_${org_id}_landing 
                #(tag_date, span, start_date, end_date, landing)
            #Values ('${vDate}' + interval 1 day, 'monthly', '${vMonthFirst}', '${vDate}', '#')
            ;
            ALTER TABLE ${project_name}.${type}_${table_name}_${org_id}_landing AUTO_INCREMENT = 1
            ; 
    ...
    do 
        ...
        then 
			...
			100 prop, 
                ...
                # add the no data domain on seasonal ones
                INSERT IGNORE INTO ${project_name}.${type}_${table_name}_${org_id}_landing 
                    (tag_date, span, start_date, end_date, landing)
                    
                    select '${vDate}' + interval 1 day, 'seasonal', '${vDate}' + interval 1 day - interval 3 month, '${vDate}', b.domain
                    from (
                        select *
                        from ${project_name}.${type}_${table_name}_${org_id}_landing
                        where tag_date = '${vDate}' + interval 1 day
                ...
                #INSERT IGNORE INTO ${project_name}.${type}_${table_name}_${org_id}_landing 
                    #(tag_date, span, start_date, end_date, landing)
                #Values ('${vDate}' + interval 1 day, 'seasonal', '${vDate}' + interval 1 day - interval 3 month, '${vDate}', '#')
                ;
                ALTER TABLE ${project_name}.${type}_${table_name}_${org_id}_landing AUTO_INCREMENT = 1
                ; 
