    ...
    export sql_3="
        UPDATE ${project_name}.${type}_${table_name}_${org_id} a
            ...
            ON a.tag_date = b.tag_date + interval 1 day
                ...
                and a.span = 'daily'
        SET a.session_gr = ifnull(100 * round((a.session - b.session) / b.session, 2), 0), 
            a.user_gr = ifnull(100 * round((a.user - b.user) / b.user, 2), 0), 
	    a.reg_gr = ifnull(100 * round((a.reg - b.reg) / b.reg, 2), 0), 
            a.page_view_gr = ifnull(100 * round((a.page_view - b.page_view) / b.page_view, 2), 0),  
            a.event_gr = ifnull(100 * round((a.event - b.event) / b.event, 2), 0),  
            a.stay_time_gr = ifnull(100 * round((a.stay_time - b.stay_time) / b.stay_time, 2), 0),  
            a.bounce_rate_gr = ifnull(100 * round((a.bounce_rate - b.bounce_rate) / b.bounce_rate, 2), 0) 	
