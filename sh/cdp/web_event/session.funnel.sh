...
do 
    ...
    export sql_1="   
	    ...
	    layer_id int NOT NULL COMMENT '行銷漏斗的階層, 最低1-大多至5-最高10', 
            freq int DEFAULT NULL COMMENT '行銷漏斗各 level 的數量',
    ...
    export sql_4="   
	    ...
	    layer_name varchar(128) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '行銷漏斗的階層名稱',
            freq int DEFAULT NULL COMMENT '各行銷漏斗 level 的數量',
            conversion_rate int DEFAULT NULL COMMENT '行銷漏斗各層轉換率(由低至高)',
    ...
    export sql_5="
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}
            select 
                ...
                c.layer_id, 
		layer_name, 
                ifnull(a.freq, 0) freq, 
                ifnull(100 * round(a.freq / ifnull(b.freq, a.freq), 2), 0) conversion_rate, 
	...
	; 
        ...
        SET conversion_overall = 
            ...
            from (
                select 
                    max(if(layer_id = 5, freq, null)) layer_5, 
                    max(if(layer_id = 1, freq, null)) layer_1
	...
	;
        ...
        UPDATE ${project_name}.${type}_${table_name}_${org_id} a
            ...
            select 
                tag_date, 
                funnel_id, 
                max(if(layer_id = 1, freq, null)) layer_min, 
                max(if(layer_name like '完成%', freq, null)) layer_max
            from ${project_name}.${type}_${table_name}_${org_id}
            where tag_date = '${vDate}' + interval 1 day
		and funnel_id >= 2
            ...
            on a.tag_date = b.tag_date
                and a.funnel_id = b.funnel_id
        SET conversion_overall = 100 * round(layer_max / layer_min, 5)
        ;"
    echo ''
    echo [CREATE TABLE IF NOT EXISTS ${project_name}.${type}_${table_name}_${org_id}]
