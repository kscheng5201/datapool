...
# Import Data 
echo ''
echo [start Import Data on ${dest_login_path}_prod]
echo 'start: ' `date`
...
export sql_2="
    insert into codebook_cdp.organization_domain
        select null, org_id, db_id, domain, domain_type, nickname, created_at
        from instagram.organization_domain
    ;
    
    insert into codebook_cdp.organization_domain
        select null, 3, 0, domain, domain_type, nickname, created_at
        from codebook_cdp.organization_domain
        where org_id = 11
            and domain_type = 'app'
    ;"
echo ''
echo [insert into codebook_cdp.organization_domain by instagram and app]
