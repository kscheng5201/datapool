...
do
    ...
    do    
        ...
        echo $sql_2
        mysql --login-path=$src_login_path -e "$sql_2" > $export_dir/$src_login_path/$project_name/${project_name}.${table_name}_keyword_${org_id}.txt 2>>$error_dir/$src_login_path/$project_name/${project_name}.${table_name}_keyword_${org_id}.error
        
        # Import Data
        echo ''
        echo 'start: ' `date`
        echo [LOAD DATA LOCAL INFILE '$export_dir/$src_login_path/$project_name/${project_name}.${table_name}_keyword_${org_id}.txt' INTO TABLE ${project_name}.${table_name}_${org_id}_etl ]]
        mysql --login-path=$dest_login_path -e "LOAD DATA LOCAL INFILE '$export_dir/$src_login_path/$project_name/${project_name}.${table_name}_keyword_${org_id}.txt' INTO TABLE ${project_name}.${table_name}_${org_id}_etl IGNORE 1 LINES;" 2>>$error_dir/${src_login_path}/${project_name}/${project_name}.${table_name}_keyword_${org_id}.error 
        ...
        sql_3="
            ...
            select 
                ...
                fpc,
                null accu_id, 
                (select domain from cdp_organization.organization_domain where org_id = ${org_id} and db_id = ${db_id} and domain_type = 'web') domain,
		...
		################################################
		# at this point, inner join fpc_event_raw_data might lose up to 50% event tag from user_tag
		# even user_tag mark which is originated event (campaign)
