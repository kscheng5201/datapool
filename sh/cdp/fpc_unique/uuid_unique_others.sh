    ...
    sed -i '1d' $export_dir/$src_login_path/$project_name/$project_name.${src_login_path}_domain_type_${i}.txt
		...
		; 
                select 
                    ...
                    'unique_id' id_type, 
                    '${q}' channel_type, 
                    (select domain from cdp_organization.organization_domain where org_id = 1 and domain_type = '${q}') domain,
                    FROM_UNIXTIME(${q}_unique_created_at) created_at, 
                    FROM_UNIXTIME(updated_at) updated_at
                from cdp_${q}_${p}.${q}_unique_data
                where ${q}_unique_id is not null
                    and ${q}_unique_id <> ''
                    and ((${q}_unique_created_at >= UNIX_TIMESTAMP('${vDate}') 
                    and ${q}_unique_created_at < UNIX_TIMESTAMP('${vDate}' + interval 1 day))
