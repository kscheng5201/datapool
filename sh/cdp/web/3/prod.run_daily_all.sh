...
sh /root/datapool/sh/cdp/web/3/prod.session.both.sh ${vDate}
#mysql --login-path=datapool -e "INSERT INTO ad.session_both_3_src_hist_new select null, date(created_at) + interval 1 day,fpc, created_at, domain, behavior, traffic_type, referrer, campaign, source_medium, event_type, page_title, page_url, session, updated_at from web.session_both_3_src;"
#mysql --login-path=datapool -e "INSERT INTO ad.session_both_3_etl_hist_new select null, date(created_at) + interval 1 day,fpc, created_at, domain, behavior, traffic_type, referrer, campaign, source_medium, event_type, page_title, page_url, session, updated_at from web.session_both_3_etl;"

#sh /root/datapool/sh/cdp/web/3/prod.session.kpi.sh ${vDate}
sh /root/datapool/sh/cdp/web/3/prod.session.page_1228.sh ${vDate}

#sh /root/datapool/sh/cdp/web/3/prod.both.onliner.sh ${vDate}
sh /root/datapool/sh/cdp/web/3/prod.person.page_1228.sh ${vDate}

#sh /root/datapool/sh/cdp/web/3/prod.both.event.sh ${vDate}

#sh /root/datapool/sh/cdp/web/3/prod.session.funnel.sh ${vDate}
