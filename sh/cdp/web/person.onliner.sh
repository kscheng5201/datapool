...
do 
    export sql_1="   
        CREATE TABLE IF NOT EXISTS ${project_name}.${type}_${table_name}_${org_id} (
            ...
            span varchar(8) NOT NULL COMMENT 'daily(yearweek)/monthly/seasonal', 
            ...
            user int DEFAULT NULL COMMENT '不重複 user 的數量',  
	    time_flag varchar(16) DEFAULT NULL COMMENT 'last: 上期; last_last: 上上期', 
            ...
            key idx_dow (dow), 
            key idx_hour (hour)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='綜合儀表板【長期統計】熱門時段'
        ;"
    echo ''
	...
	# UPDATE the time_flag on week
        UPDATE ${project_name}.${type}_${table_name}_${org_id}
        SET time_flag = if(span = yearweek('${vDate}', 7), 'last', null)
        ...
        export sql_5="
		    ...
		    null time_flag,  
                ...
                from (   
                    ...
                    from (
                        select tag_date, left(dayname(tag_date), 3) dow
                        from ${project_name}.${type}_${table_name}_${org_id}
                        where start_date >= date_format('${vDate}', '%Y-%m-01')
                            and start_date < '${vDate}' + interval 1 day 
                        group by tag_date, left(dayname(tag_date), 3)
                        ) b
    ...
    do
			...
			null time_flag, 
                    ...
                    from (   
                        select 
                            ...
                            sum(person) person
                        from ${project_name}.${type}_${table_name}_${org_id}
                        where start_date >= '${seasonDate}' + interval 1 day - interval 3 month
                            and start_date < '${seasonDate}' + interval 1 day
                            and span not in ('monthly', 'seasonal')
                        group by 
                            domain, 
                            dow, 
		...
		# UPDATE the time_flag on season
                UPDATE ${project_name}.${type}_${table_name}_${org_id}
                SET time_flag = if(tag_date = '${vDate}' + interval 1 day, 'last', null)
                WHERE span = 'seasonal'
                ;"
