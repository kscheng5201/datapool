...
do 
    ...
    export sql_2a="  
        ...
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_landing
            select 
                ...
                now() updated_at
            from (
                select domain, traffic_type, row_number () over (partition by fpc, session order by created_at) rid
    ...
    export sql_2b="
        ...
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_traffic
            select 
                ...
                now() updated_at
            from (
                select domain, traffic_type, count(distinct fpc, session) valid
                from (
                    select domain, traffic_type, fpc, session, row_number () over (partition by domain, traffic_type, fpc, session order by created_at) rid
                    from ${project_name}.${type}_both_${org_id}_etl
                ...
                left join
                (
                select domain, traffic_type, count(distinct fpc, session) invalid
                from (
                    select domain, traffic_type, fpc, session, row_number () over (partition by domain, traffic_type, fpc, session order by created_at) rid
                    from ${project_name}.${type}_both_${org_id}_etl
        ...
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_traffic
            select 
                ...
                now() updated_at
            from (
                select domain, traffic_type, referrer, count(distinct fpc, session) valid
                from (
                    select domain, traffic_type, referrer, fpc, session, row_number () over (partition by domain, traffic_type, fpc, session order by created_at) rid
                    from ${project_name}.${type}_both_${org_id}_etl
                    where session_type = 'valid'
                    ) a
                where rid = 1
                ...
                left join
                (
                select domain, traffic_type, referrer, count(distinct fpc, session) invalid
                from (
                    select domain, traffic_type, referrer, fpc, session, row_number () over (partition by domain, traffic_type, fpc, session order by created_at) rid
                    from ${project_name}.${type}_both_${org_id}_etl
                    where session_type = 'invalid'
        ...
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_traffic
            select 
                ...
                now() updated_at
            from (
                select traffic_type, referrer, count(distinct fpc, session) valid
                from (
                    select traffic_type, referrer, fpc, session, row_number () over (partition by traffic_type, fpc, session order by created_at) rid
                    from ${project_name}.${type}_both_${org_id}_etl
                    where session_type = 'valid'
                    ) a
                where rid = 1
                ...
                left join
                (
                select traffic_type, referrer, count(distinct fpc, session) invalid
                from (
                    select traffic_type, referrer, fpc, session, row_number () over (partition by traffic_type, fpc, session order by created_at) rid
                    from ${project_name}.${type}_both_${org_id}_etl
                    where session_type = 'invalid'
    ...
    export sql_2c="
        ...
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_campaign
            select 
                ...
                now() updated_at
            from (
                select domain, campaign, count(distinct fpc, session) valid
                from ${project_name}.${type}_both_${org_id}_etl
                    and campaign not in (null, 'NULL', '')
                where session_type = 'valid'
                ...
                left join
                (
                select domain, campaign, count(distinct fpc, session) invalid
                from ${project_name}.${type}_both_${org_id}_etl
                where session_type = 'invalid'
                    and behavior = 'page_view'
                    and traffic_type = 'Ad'
        ...
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_campaign
            select 
                ...
                now() updated_at
            from (
                select campaign, count(distinct fpc, session) valid
                from ${project_name}.${type}_both_${org_id}_etl
                where session_type = 'valid'
                    and behavior = 'page_view'
                    and traffic_type = 'Ad'
                ...
                left join
                (
                select campaign, count(distinct fpc, session) invalid
                from ${project_name}.${type}_both_${org_id}_etl
                where session_type = 'invalid'
                    and behavior = 'page_view'
                    and traffic_type = 'Ad'
    ...
    export sql_2d="
        ...
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_source_medium
            select 
                null serial, 
                '${vDate}' + interval 1 day tag_date, 
                'daily' span, 
                ...
                now() updated_at
            from (
                select domain, source_medium, count(distinct fpc, session) valid
                from ${project_name}.${type}_both_${org_id}_etl
                where session_type = 'valid'
                    and behavior = 'page_view'
                    and traffic_type = 'Ad'
                ...
                left join
                (
                select domain, source_medium, count(distinct fpc, session) invalid
                from ${project_name}.${type}_both_${org_id}_etl
                where session_type = 'invalid'
                    and behavior = 'page_view'
                    and traffic_type = 'Ad'
        ...
        ;
        ALTER TABLE ${project_name}.${type}_${table_name}_${org_id}_source_medium AUTO_INCREMENT = 1
        ;
        ...
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_source_medium
            select 
                ...
                now() updated_at
            from (
                select source_medium, count(distinct fpc, session) valid
                from ${project_name}.${type}_both_${org_id}_etl
                where session_type = 'valid'
                    and behavior = 'page_view'
                    and traffic_type = 'Ad'
                ...
                left join
                (
                select source_medium, count(distinct fpc, session) invalid
                from ${project_name}.${type}_both_${org_id}_etl
                where session_type = 'invalid'
                    and behavior = 'page_view'
                    and traffic_type = 'Ad'
        ...
        ;"
    echo ''
    echo [INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_source_medium]
    echo $sql_2d
    mysql --login-path=$dest_login_path -e "$sql_2d" 2>>$error_dir/$project_name/${project_name}.${type}_${table_name}_${org_id}_source_medium_daily.error 
    ...
    export sql_2f="
        ...
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_title
            select 
                ...
                now() updated_at
            from (
                select domain, page_title, count(distinct fpc, session) valid
                from ${project_name}.${type}_both_${org_id}_etl
                where session_type = 'valid'
                ...
                left join
                (
                select domain, page_title, count(distinct fpc, session) invalid
                from ${project_name}.${type}_both_${org_id}_etl
                where session_type = 'invalid'
                    and page_title is not null
                    and page_title <> 'NULL'
        ...
        ;
        ALTER TABLE ${project_name}.${type}_${table_name}_${org_id}_page_title AUTO_INCREMENT = 1
        ;
        ...
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_title
            select 
                ...
                now() updated_at
            from (
                select page_title, count(distinct fpc, session) valid
                from ${project_name}.${type}_both_${org_id}_etl
                where session_type = 'valid'
                    and page_title is not null
                    and page_title <> 'NULL'
                ...
                left join
                (
                select page_title, count(distinct fpc, session) invalid
                from ${project_name}.${type}_both_${org_id}_etl
                where session_type = 'invalid'
                    and page_title is not null
                    and page_title <> 'NULL'
    ...
    echo $sql_2f
    mysql --login-path=$dest_login_path -e "$sql_2f" 2>>$error_dir/$project_name/${project_name}.${type}_${table_name}_${org_id}_page_title_daily.error 
...
done < $export_dir/$src_login_path/$project_name/$project_name.${src_login_path}_org_id_16.txt
echo ''
