...
do 
    ...
    mysql --login-path=$dest_login_path -e "DROP TABLE IF EXISTS ${project_name}.${type}_${table_name}_${org_id}_etl_pre;"
    ...
    do 
        ...
        do 
            export sql_5="
                ...
                where created_at >= UNIX_TIMESTAMP(date_format(now(), '%Y-%m-%d %H:00:00') - interval 1 day) 
                    and created_at < UNIX_TIMESTAMP(date_format(now(), '%Y-%m-%d %H:00:00') - interval 1 day + interval 1 day)
        ...
        export sql_9="
            ...
            where a.created_at >= UNIX_TIMESTAMP(date_format(now(), '%Y-%m-%d %H:00:00') - interval 1 day) 
                and a.created_at < UNIX_TIMESTAMP(date_format(now(), '%Y-%m-%d %H:00:00') - interval 1 day + interval 1 day)
            ;"
        echo $sql_9
        #### Export Data ####
        echo ''
    ...
    export sql_11="  
        ...
        ;
        CREATE TABLE IF NOT EXISTS ${project_name}.${type}_${table_name}_${org_id}_etl_pre (
            serial int auto_increment NOT NULL COMMENT 'auto_increment Serial Number' unique, 
            fpc varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '瀏覽器指紋碼 fingerprint code',
            created_at datetime NOT NULL COMMENT '網頁瀏覽或事件觸發的時間戳記', 
            domain varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '來源 domain', 
...
#    do 
        ...
        export sql_14bb="
            INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_etl_b  
                select 
                    ...
                    a.page_title, 
                    a.page_url,
                    if(timestampdiff(day, b.created_at, a.created_at) >= 30, 1, 0) session_pre,
                    now() updated_at                
                from ${project_name}.${type}_${table_name}_${org_id}_etl_a a
                    left join ${project_name}.${type}_${table_name}_${org_id}_etl_a b
        ...
        echo $sql_14bb
        mysql --login-path=$dest_login_path -e "$sql_14bb" 2>>$error_dir/$src_login_path/$project_name/${project_name}.${type}_${table_name}_${org_id}_sql14bb.error
        ...
        export sql_14bd="
            INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_etl_pre
                select 
                    null serial, 
                    fpc, 
                    created_at, 
