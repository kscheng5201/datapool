...
do 
    ...
    echo ''
    echo [DROP TABLE IF EXISTS ${project_name}.${type}_${table_name}_${org_id}_temp]
    mysql --login-path=$dest_login_path -e "DROP TABLE IF EXISTS ${project_name}.${type}_${table_name}_${org_id}_temp" 2>>$error_dir/$src_login_path/$project_name/$project_name.${type}_${table_name}_${org_id}_temp_drop.error 
		...
		count(*) user, 
            ...
            from (
                select
                    ...
                    row_number () over (partition by accu_id, session, domain order by layer_id) rid
    ...
    do 
            ...
            do 
                export sql_15="
                    INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_temp    
                        ...
                        from (
                            select 
                                ifnull(e.accu_id, e.fpc) accu_id, 
                                e.domain, 
                                type, 
                                event, 
                                col1, 
                            ...
                            from ${project_name}.${type}_event_${org_id}_src e
                                inner join uuid.accu_mapping_${org_id} g
                                    on e.fpc = g.id 
                            where id_type = 'fpc'
                                and type = $(echo ${ev_layer} | cut -d _ -f 1)
                            ) a
                            
                            inner join
                            (
                            select accu_id, domain, created_at, session
                            from ${project_name}.${type}_both_${org_id}_etl
                    ...
                    INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_temp    
                        ...
                        from (
                            select 
                                ...
                                e.created_at
                            from ${project_name}.${type}_event_${org_id}_src e
                                right join uuid.accu_mapping_${org_id} g
                                    on e.fpc = g.id 
                            where id_type = 'fpc'
                                and type = $(echo ${ev_layer} | cut -d _ -f 1)
                                and e.accu_id is null
                            ) a
                ...
                mysql --login-path=$dest_login_path -e "$sql_15" 2>>$error_dir/$src_login_path/$project_name/${project_name}.${type}_${table_name}_${org_id}_temp_sql_15.error
        ...
        mysql --login-path=$dest_login_path -e "$sql_16" 2>>$error_dir/$src_login_path/$project_name/${project_name}.${type}_${table_name}_${org_id}_temp_sql_16.error
    ...
    echo ''
    echo [DROP TABLE IF EXISTS ${project_name}.${type}_${table_name}_${org_id}_temp]
    mysql --login-path=$dest_login_path -e "DROP TABLE IF EXISTS ${project_name}.${type}_${table_name}_${org_id}_temp" 2>>$error_dir/$src_login_path/$project_name/$project_name.${type}_${table_name}_${org_id}_temp_drop_af_sql_17.error 
	...
	;
        ...
        ;"
    echo ''
    echo [INSERT INTO and UPDATE ${project_name}.${type}_${table_name}_${org_id}]
    echo $sql_19
