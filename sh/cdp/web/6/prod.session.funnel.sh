...
echo ''
echo [Get the org_id]
#mysql --login-path=$src_login_path -e "$sql_0" > $export_dir/$src_login_path/$project_name/$project_name.${src_login_path}_org_id_6.txt 2>>$error_dir/$src_login_path/$project_name/$project_name.${src_login_path}_org_id_6.error
...
do 
    ...
    echo "INSERT INTO tag.${type}_${table_name}_${org_id}_temp"
    echo "DROP TABLE ${project_name}.${type}_${table_name}_${org_id}_temp;"
    mysql --login-path=$dest_login_path -e "INSERT INTO tag.${type}_${table_name}_${org_id}_temp select null, tag_date, domain, fpc, concat(left(session, 8), '00'), funnel_id, max(layer_id), now() created_at, now() updated_at from ${project_name}.${type}_${table_name}_${org_id}_temp group by tag_date, domain, fpc, funnel_id;"
...
#done < /root/datapool/export_file/cdp/web/web.cdp_org_id_6.txt
echo ''
echo 'end: ' `date`
