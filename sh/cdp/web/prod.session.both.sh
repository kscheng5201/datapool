...
do 
    ...
    mysql --login-path=$dest_login_path -e "DROP TABLE IF EXISTS ${project_name}.${type}_${table_name}_${org_id}_etl_break;"  
    ...
    export sql_11="  
	    ...
	    key idx_accu_id (accu_id),
            ...
            key idx_traffic_type (traffic_type),
            key idx_referrer (referrer),
	    key idx_session_pre (session_pre)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='網頁瀏覽與事件觸發的 etl 整合表（加入 accu_id)'
        ;
    ...
    export sql_14bc="
        create table ${project_name}.${type}_${table_name}_${org_id}_etl_break (
            accu_id varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'accu_id',
            start_at datetime NOT NULL COMMENT 'session 開始時間',
            end_at datetime DEFAULT NULL COMMENT 'session 結束時間',    
            session_break int NOT NULL COMMENT 'session 編號',     
            primary key (accu_id, start_at),
            key idx_accu_id (accu_id),
	    key idx_session_break (session_break)
        )
        ;
        
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_etl_break    
            select accu_id, min(created_at), null, 1 session_break
            from ${project_name}.${type}_${table_name}_${org_id}_etl_b
            group by accu_id
        ;
        
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_etl_break
            select accu_id, created_at, null, row_number() over (partition by accu_id order by created_at) + 1 session_break
            from ${project_name}.${type}_${table_name}_${org_id}_etl_b
            where session_pre = 1
        ;


        UPDATE ${project_name}.${type}_${table_name}_${org_id}_etl_break a
            inner join ${project_name}.${type}_${table_name}_${org_id}_etl_break b
            on a.accu_id = b.accu_id
                and a.session_break = b.session_break - 1
        SET a.end_at = b.start_at
        ;
        
        UPDATE ${project_name}.${type}_${table_name}_${org_id}_etl_break
        SET end_at = date(start_at) + interval 1 day
        WHERE end_at is null
        ;


        UPDATE ${project_name}.${type}_${table_name}_${org_id}_etl_b a
            INNER JOIN ${project_name}.${type}_${table_name}_${org_id}_etl_break b
            on a.accu_id = b.accu_id
        SET session_pre = session_break
        WHERE a.created_at >= start_at
            and a.created_at < end_at
        ;"
    echo ''
    echo [start: `date` on ${vDate}]
    ...
    export sql_14bd="
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_etl
            select 
                ...
                page_title, 
                page_url, 
                concat(date_format(created_at, '%Y%m%d'), LPAD(session_pre, 2 ,'0')) session, 
                null session_type,
                now() updated_at
            from ${project_name}.${type}_${table_name}_${org_id}_etl_b
        ;"
