...
do 
    export sql_1="  
        ...
        create table if not exists ${project_name}.${type}_${table_name}_${org_id}_domain_fpc (   
            ...
            key idx_landing (landing), 
            key idx_traffic_type (traffic_type)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='原始 fpc on 綜合儀表板【頁面深度分析】-熱門頁面-瀏覽網域'
        ; 
        create table if not exists ${project_name}.${type}_${table_name}_${org_id}_traffic_fpc (   
            ...
            key idx_referrer (referrer), 
            key idx_created_at (created_at)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='原始 fpc on 綜合儀表板【頁面深度分析】導流分析-流量種類/自然流量/其它流量'
        ;
        create table if not exists ${project_name}.${type}_${table_name}_${org_id}_campaign_fpc (
            ...
            key idx_start_date (start_date), 
            key idx_fpc (fpc)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='原始 fpc on 綜合儀表板【頁面深度分析】廣告流量-廣告活動'
        ;        
       create table if not exists ${project_name}.${type}_${table_name}_${org_id}_title_fpc (   
            ...
            key idx_start_date (start_date), 
            key idx_fpc (fpc)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='原始 fpc on 綜合儀表板【頁面深度分析】熱門頁面-頁面標題'
        ;"
    echo ''
    ...
    export sql_2a="  
        ...
        INSERT INTO ${project_name}.${type}_${table_name}_${org_id}_domain_fpc
            select 
                ...
                now() updated_at
            from (
                select fpc, session, domain landing, traffic_type
                from (
    ...
    echo $sql_2a
    mysql --login-path=$dest_login_path -e "$sql_2a" 2>>$error_dir/$project_name/${project_name}.${type}_${table_name}_${org_id}_domain_fpc_daily.error 
    ...
    echo $sql_2b
    mysql --login-path=$dest_login_path -e "$sql_2b" 2>>$error_dir/$project_name/${project_name}.${type}_${table_name}_${org_id}_traffic_fpc_daily.error 
    ...
    echo $sql_2c
    mysql --login-path=$dest_login_path -e "$sql_2c" 2>>$error_dir/$project_name/${project_name}.${type}_${table_name}_${org_id}_campaign_fpc_daily.error 
    ...
    do 
        ...
        then 
            ...
            echo $sql_7
            mysql --login-path=$dest_login_path -e "$sql_7" 2>>$error_dir/$project_name/${project_name}.${type}_${table_name}_${org_id}_anything_delete.error 
