...
export table_name="benson_event" 
    ...
    export sql_11a="
        INSERT INTO ${project_name}.${type_s}_${table_name}_${org_id}
            select 
                ...
                attribute, 
                content, 
                count(distinct fpc, session) freq,
                row_number () over (partition by domain, event, attribute order by count(distinct fpc, session) desc) ranking, 
                null time_flag,
                now() created_at, 
                now() updated_at
        ...
        INSERT INTO ${project_name}.${type_s}_${table_name}_${org_id}
            select 
                ...
                content, 
                count(distinct fpc, session) freq,
                row_number () over (partition by event, attribute order by count(distinct fpc, session) desc) ranking, 
                null time_flag,
                now() created_at, 
                now() updated_at
            from ${project_name}.${type_s}_${table_name}_${org_id}_pre
