...
do
    export sql_query_0="
        ...
        where created_at < date(now()) and id in (126, 182)
#            and deleted_at is null
    ...
    do
        export sql_query_1="
            CREATE TABLE IF NOT EXISTS ${project_name}.${cur_im}bot_${p}_interaction (   
            ...
            ;
            CREATE TABLE IF NOT EXISTS ${project_name}.${cur_im}bot_${p}_lifespan (  
                serial int auto_increment NOT NULL COMMENT 'auto_increment Serial Number' unique,
                user_id int NOT NULL COMMENT 'user id',
        ...
        echo ''
        echo 'start: ' `date` 
        echo create table if not exists ${project_name}.${cur_im}bot_${p}_interaction and interaction
        ...
        export sql_query_2="
            ...
            FROM accunix_v2_log.${cur_im}bot_${p}_users
               WHERE created_at < '${vDate}' + interval 1 day
            ;"
	echo ''
	echo $sql_query_2
	...
	echo $sql_query_3
        ...
        echo ''
        echo 'start: ' `date` 
        echo ${project_name}.${cur_im}bot_${p}_interaction_first
        mysql --login-path=$src_login_path -e "$sql_query_2" > $export_dir/$src_login_path/$project_name/$project_name.${cur_im}bot_${p}_interaction_first.txt 2>>$error_dir/$src_login_path/$project_name/$project_name.${cur_im}bot_${p}_interaction_first.error
        mysql --login-path=$dest_login_path -e "LOAD DATA LOCAL INFILE '$export_dir/$src_login_path/$project_name/$project_name.${cur_im}bot_${p}_interaction_first.txt' INTO TABLE ${project_name}.${cur_im}bot_${p}_interaction IGNORE 1 LINES (user_id, interaction_date, first_date, created_at);" 2>>$error_dir/$project_name.${cur_im}_${p}_interaction_first.error 

        # Export Data
        echo ''
        echo 'start: ' `date` 
        echo [INSERT INTO ${project_name}.${cur_im}bot_${p}_interaction]
        mysql --login-path=$src_login_path -e "$sql_query_3" > $export_dir/$src_login_path/$project_name/$project_name.${cur_im}bot_${p}_interaction.txt 2>>$error_dir/$src_login_path/$project_name/$project_name.${cur_im}bot_${p}_interaction.error
        mysql --login-path=$dest_login_path -e "LOAD DATA LOCAL INFILE '$export_dir/$src_login_path/$project_name/$project_name.${cur_im}bot_${p}_interaction.txt' IGNORE INTO TABLE ${project_name}.${cur_im}bot_${p}_interaction IGNORE 1 LINES (user_id, interaction_date, created_at);" 2>>$error_dir/$project_name.${cur_im}_${p}_interaction.error 


        export sql_query_4="        
            UPDATE ${project_name}.${cur_im}bot_${p}_interaction a
                LEFT JOIN
                (
                select user_id, min(interaction_date) first_date
                from ${project_name}.${cur_im}bot_${p}_interaction
                where user_id in (
                    select user_id
                    from ${project_name}.${cur_im}bot_${p}_interaction
	...
	echo [start ${vDate} date at `date`] 
        echo [update ${project_name}.${cur_im}bot_${p}_interaction]
        mysql --login-path=$dest_login_path -e "$sql_query_4"


        export sql_5="
            UPDATE ${project_name}.${cur_im}bot_${p}_lifespan a
                LEFT JOIN
                (
        ...
        echo ''
        echo [start ${vDate} date at `date`] 
        echo [UPDATE ${project_name}.${cur_im}bot_${p}_lifespan]
        mysql --login-path=$dest_login_path -e "$sql_5"

        export sql_6="
            INSERT INTO ${project_name}.${cur_im}bot_${p}_lifespan
                select 
                    null serial, 
                    b.*, 
                    now() created_at, 
                    now() updated_at
                from ${project_name}.${cur_im}bot_${p}_lifespan a
                    RIGHT JOIN
                    (
                    select 
                        user_id, 
        ...
        echo ''
        echo [start ${vDate} date at `date`] 
        echo [INSERT INTO ${project_name}.${cur_im}bot_${p}_lifespan for new user_id]
        ...
        export sql_7="
            ...
            from (
                ...
                from (
                    ...
                    from ${project_name}.${cur_im}bot_${p}_lifespan
                    where cycle_time > 0 
		...
		;"
            ...
            mysql --login-path=$dest_login_path -e "$sql_7" > $export_dir/$src_login_path/$project_name/$project_name.${src_login_path}_${p}_active_days.txt 2>>$error_dir/$src_login_path/$project_name/$project_name.${src_login_path}_${p}_active_days.error
            sed -i '1d' $export_dir/$src_login_path/$project_name/$project_name.${src_login_path}_${p}_active_days.txt
		...
		mysql --login-path=$dest_login_path -e "$sql_8" 2>>$error_dir/$src_login_path/$project_name/$project_name.${project_name}.${cur_im}bot_lifecycle_days.error
            ...
            done < $export_dir/$src_login_path/$project_name/$project_name.${src_login_path}_${p}_active_days.txt
